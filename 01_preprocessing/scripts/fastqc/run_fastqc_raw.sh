...
# Description   : Run FastQC on raw paired-end reads in parallel
# Author        : Layan Essam
...
FASTQC_OUTPUT_DIR="$FASTQC_RAW_DIR"
mkdir -p "$FASTQC_OUTPUT_DIR"
LOG_FILE="$FASTQC_OUTPUT_DIR/${FASTQC_TYPE}_${TIMESTAMP}.log"
...

{
  ...
  if [[ "$SUCCESS_COUNT" -eq "$NUM_SAMPLES" ]]; then
    echo "✅ Step completed successfully: run_fastqc_raw"
  else
